...
if (not op.exists(input_image)) or (not op.exists(mask_image)):
...
save(dom_img, op.join(write_dir, "myroi.nii"))
print 'Wrote an ROI mask image in %s' % op.join(write_dir, "myroi.nii")
...
blobPath = op.join(write_dir, "blob.nii")
save(wim, blobPath)
...
roi.select_roi([132])
wim2 = roi.to_image()
roi_path_2 = op.join(write_dir, "roi_blob_1.nii")
...
wim3 = roi.to_image()
roi_path_3 = op.join(write_dir, "blob_closest_to_%d_%d_%d.nii"
...
wim4 = roi.to_image()
roi_path_4 = op.join(write_dir, "roi_all_blobs.nii")
...
wim5 = roi.to_image()
roi_path_5 = op.join(write_dir, "roi_some_blobs.nii")
