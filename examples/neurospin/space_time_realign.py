...
print runnames
images = [brifti.load(rname) for rname in runnames]
...
corr_img = brifti.nifti1.Nifti1Image(corr_run.get_data(), corr_run.get_affine())
brifti.save(corr_img, 'corr_run1.nii')
"""
...
corr_images = [brifti.nifti1.Nifti1Image(run.get_data(), run.get_affine()) for run in corr_runs]

# Save images 
for i in range(len(runs)):
    aux = split(runnames[i])
    brifti.save(corr_images[i], join(aux[0], 'ra'+aux[1]))
