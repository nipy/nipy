...
for subj in range(16):
    ...
    os.system('python2.4 /home/analysis/FIAC/python/subject.py %d' % subj)
    ttic = time.time()
    ...
    for design in ['block', 'event']:
        for contrast in ['overall', 'speaker', 'sentence', 'interaction']:
            for stat in ['t', 'sd', 'effect']:
                ...
                os.system('python2.4 /home/analysis/FIAC/python/plots-fixed.py --which=%s --contrast=%s --design=%s --stat=%s' %(which, contrast, design, stat))
...
for what in ['rho', 'fwhmOLS']:
    os.system('python2.4 /home/analysis/FIAC/python/plots-run.py --what=%s' % what)
ttic = time.time()
...
for which in ['delays', 'contrasts']:
    for design in ['block', 'event']:
        for contrast in ['overall', 'speaker', 'sentence', 'interaction']:
            cmd = """
            python2.4 /home/analysis/FIAC/python/multi.py --which=%s
