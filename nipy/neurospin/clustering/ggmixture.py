...
class GGGM(object):
    ...
    def SAEM(self, x, burnin=100, niter=200, verbose=False):
        ...
        niter: integer, optional
               maximum number of SAEM iterations for convergence
        ...
        for t in xrange(burnin + niter):
            ...
            def Qp(Y):
                ...
                """
                return A[6] * (1 - Y[0]) + A[3] * Y[1] - A[1] * \
                    (Y[0] * np.log(Y[1]) - np.log(sp.gamma(Y[0])))

            def Qn(Y):
                """
                Expected log_likelihood of negative gamma class
                """
                return A[7] * (1 - Y[0]) + A[4] * Y[1] - A[2] * \
                    (Y[0] * np.log(Y[1]) - np.log(sp.gamma(Y[0])))
    ...
    def ROC(self, x):
        ...
        #False positives
        P[0] = (
            self.mixt[0] * st.gamma.cdf( - x, self.shape_n, scale=self.scale_n)
