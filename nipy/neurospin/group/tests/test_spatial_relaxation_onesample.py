...
class test_multivariate_stat_saem(unittest.TestCase):
    ...
    def test_evaluate_exact(self):
        ...
        P.evaluate(nsimu=100, burnin=100, J = [XYZvol[5,5,5]], compute_post_mean=True, verbose=True)
        ...
        Q.evaluate(nsimu=100, burnin=100, J = [XYZvol[5,5,5]], compute_post_mean=True, update_spatial=False, verbose=True)
    ...
    def test_model_selection(self):
        data, XYZ, mask, XYZvol, vardata, signal = make_data(n=30, dim=30, r=3, mdim=10, maskdim=10, amplitude=0.1, noise=1, jitter=1, activation=False)
        #P1 = os.multivariate_stat(data[:, mask], v_shape=0.01, v_scale=0.01, std_shape=0.01, std_scale=0.01, m_mean_rate=0.01, m_var_shape=0.01, m_var_scale=0.01)
        P1 = os.multivariate_stat(data[:, mask])
        P1.init_hidden_variables()
        P1.evaluate(nsimu=1000, burnin=100, verbose=True)
        #v = P1.v.copy()
        #m_var = P1.m_var.copy()
        #m_mean = P1.m_mean.copy()
        #L1 = P1.compute_log_region_likelihood(v, m_mean, m_var)[0]
        #Prior1 = P1.compute_log_prior(v, m_mean, m_var)[0]
        #Post1 = P1.compute_log_posterior(v, m_mean, m_var, nsimu=1e3, burnin=0, verbose=True)[0]
        L1 = P1.compute_log_region_likelihood()[0]
        Prior1 = P1.compute_log_prior()[0]
        Post1 = P1.compute_log_posterior(nsimu=1e3, burnin=0, verbose=True)[0]
        M1 = L1 + Prior1 - Post1
        #P0 = os.multivariate_stat(data[:, mask], v_shape=0.01, v_scale=0.01, std_shape=0.01, std_scale=0.01, m_mean_rate=0.01, m_var_shape=0.01, m_var_scale=0.01)
        P0 = os.multivariate_stat(data[:, mask])
        P0.network[0] = 0
        P0.init_hidden_variables()
        P0.evaluate(nsimu=1000, burnin=100, verbose=True)
        #v = P0.v.copy()
        #m_var = P0.m_var.copy()
        #m_mean = P0.m_mean.copy()
        L0 = P0.compute_log_region_likelihood()[0]
        Prior0 = P0.compute_log_prior()[0]
        Post0 = P0.compute_log_posterior(nsimu=1e3, burnin=0, verbose=True)[0]
        M0 = L0 + Prior0 - Post0
