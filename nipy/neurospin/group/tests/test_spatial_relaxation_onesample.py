...
def make_data(n=10, dim=20, r=5, mdim=15, maskdim=20, amplitude=10, 
    ...
    I = XYZvol[(dim-mdim)/2:(dim+mdim)/2, 
               (dim-mdim)/2:(dim+mdim)/2, 
               (dim-mdim)/2:(dim+mdim)/2 ].ravel()
    mask = XYZvol[(dim-maskdim)/2:(dim+maskdim)/2, 
                  (dim-maskdim)/2:(dim+maskdim)/2, 
                  (dim-maskdim)/2:(dim+maskdim)/2 ].ravel()
    q = len(mask)
...
class TestMultivariateStatSaem(unittest.TestCase):
    ...
    def test_evaluate_exact(self):
        ...
        data, XYZ, mask, XYZvol, vardata, signal = \
                make_data(n=20, dim=20, r=3, mdim=15, maskdim=15, 
                          amplitude=5, noise=0, jitter=0, activation=False)
        XYZvol *= 0
        ...
        Q = os.multivariate_stat(data[:, mask], vardata[:, mask]*0, 
                                 XYZ[:, mask], std=0, sigma=5)
        ...
        self.assertAlmostEqual(P.mean_m.mean(), 
                               Q.mean_m.mean(), 
    ...
    def test_model_selection_exact(self):
        ...
        Post1 = P1.compute_log_posterior(nsimu=1e2, burnin=1e2, 
                                         verbose=verbose)
    ...
    def test_model_selection_mfx_spatial_rand_walk(self):
        data, XYZ, mask, XYZvol, vardata, signal = \
                make_data(n=20, dim=20, r=3, mdim=15, maskdim=15, 
                          amplitude=3, noise=1, jitter=1, activation=True)
        labels = (signal > 0).astype(int)
        P = os.multivariate_stat(data[:, mask], vardata[:, mask], 
                    XYZ[:, mask], std=2.0, sigma=5, labels=labels)
        P.network[:] = 0
        P.init_hidden_variables()
        P.evaluate(nsimu=100, burnin=100, verbose=verbose, 
                    proposal='rand_walk', proposal_std=1.0)
        ...
        Post0 = P.compute_log_posterior(nsimu=1e2, burnin=1e2, 
                    verbose=verbose)[0]
        M0 = L0 + Prior0 - Post0
        self.assertAlmostEqual(0.1*M0, 
                    0.1*P.compute_marginal_likelihood(verbose=verbose)[0], 
                    0)
        P.network[:] = 1
        ...
        P.estimate_displacements_SA(nsimu=10, c=0.99, 
                                    proposal_std=0.5, verbose=verbose)
        L1 = P.compute_log_region_likelihood()[0]
        self.assertTrue(L1 > L10)
        Prior1 = P.compute_log_prior()[0]
        Post1 = P.compute_log_posterior(nsimu=1e2, burnin=1e2, 
                                        verbose=verbose)[0]
        M1 = L1 + Prior1 - Post1
...
class TestMultivariateStatMcmc(unittest.TestCase):
    ...
    def test_evaluate_mfx_spatial(self):
        data, XYZ, mask, XYZvol, vardata, signal = make_data(n=20, 
                    dim=20, r=3, mdim=15, maskdim=15, amplitude=5, 
                    noise=1, jitter=1, activation=False)
        P = os.multivariate_stat(data[:, mask], vardata[:, mask], 
                                 XYZ[:, mask], std=1, sigma=3)
        P.init_hidden_variables()
...
class TestLabelsPrior(unittest.TestCase):
    ...
    def test_update_labels(self):
        data, XYZ, mask, XYZvol, vardata, signal = make_data(n=20, 
                        dim=20, r=3, mdim=15, maskdim=15, amplitude=5, 
                        noise=1, jitter=1, activation=True)
