...
def randthresh_main(Y,K,XYZ=None,p=np.inf,varwind=False,knownull=True,stop=False,verbose=False):
    ...
    In:  Y         (n,)    Observations
         ...
         XYZ       (3, n)  voxel coordinates. If not empty, connexity constraints are used
                           on the non-null set
         p         <float> lp norm
...
def randthresh_fixwind_gaussnull(Y,K,p=np.inf,stop=False,one_sided=False,verbose=False):
    ...
    In:  Y         (n,)    Observations (assumed Gaussian under H0, with unknown variance)
         ...
         p         <float> lp norm
         stop      <bool>  Stop when minimum is attained (save computation time)
         one_sided <bool>  If nonzero means are positive only (vs. positive or negative)
    Out: C    (n-K)   Lp norm of partial sums fluctuation about their conditional expectation
    """
    n = len(Y)
    I = 1.0/np.arange(1,n+1)
    if one_sided:
        sortY = np.sort(Y)
        std = np.sqrt((np.sum(sortY[1:K]**2) + np.cumsum(sortY[K:n]**2))*1.0/np.arange(K,n))
        std = std[::-1]
    else:
        sortY = np.sort(np.square(Y))
        V = (np.sum(sortY[1:K]) + np.cumsum(sortY[K:n]))*1.0/np.arange(K,n)
        V = V[::-1]
    C = np.zeros(n-K,float)
    sortY = sortY[::-1]
    for k in xrange(2,n - K):
        if one_sided:
            X = np.clip(-np.log(1 - ST.norm.cdf(sortY[k + 1 : k + K + 1], scale=std[k])), 0, 1 / tol)
        else:
            X = np.clip(-np.log(1 - ST.chi2.cdf(sortY[k + 1 : k + K + 1], 1, 0, scale=V[k])), 0, 1 / tol)
        # Ratio of expectations
        B = np.arange(1,K+1) * (1 + I[:n-1-k].sum() - I[:K].cumsum())
        B /= float(K) * (1 + I[K:n-1-k].sum())
        # Partial sums
...
def randthresh_varwind_gaussnull(Y,K,p=np.inf,stop=False,one_sided=False,verbose=False):
    ...
    for k in xrange(2,n - K):
        if one_sided:
            X = np.clip(-np.log(1 - ST.norm.cdf(sortY[k + 1 :], scale=std[k])), 0, 1 / tol)
        else:
            X = np.clip(-np.log(1 - ST.chi2.cdf(sortY[k + 1 :], 1, 0, scale=V[k])), 0, 1 / tol)
        # Ratio of expectations
        B = np.arange(1,n-k)*( 1 + I[:n-1-k].sum() - I[:n-k-1].cumsum() )
        B /= float(n-k-1)
        # Partial sums
...
def randthresh_fixwind_knownull_connex(X,K,XYZ,p=np.inf,stop=False,verbose=False):
    ...
    for k in xrange(2,n - K):
        ...
        Lk = np.where((XYZk.sum(axis=0) <= 2)*(XYZk.max(axis=0) <= 1))[0][:-1]
        if len(Lk) == 0:
            L[J[k-1]] = 1
        else:
            L[J[Lk]] = 0
...
def randthresh_varwind_knownull_connex(X,K,XYZ,p=np.inf,stop=False,verbose=False):
    ...
    for k in xrange(2,n - K):
        ...
        if p == np.inf:
            ...
            C[k] = ( np.abs(T - Q)**p ).sum()/(n-k-1+nk)**(p/2.0+1)
        if C[k] > C[k-1] and C[k-1] < C[k-2] and stop:
            break
        if verbose:
            print "k:", k, "nk:", nk, "C[k]:", C[k]
...
def randthresh_fixwind_gaussnull_connex(X,K,XYZ,p=np.inf,stop=False,verbose=False):
    ...
    Random threshold with fixed-window and gaussian null distribution,
    using connexity constraint on non-null set.
    In:  X (n,): Observations (assumed Gaussian under H0)
         XYZ (3,n): voxel coordinates
         K <int>: Some positive integer (lower bound on the number of null hypotheses)
         p <float>: Lp norm
         stop <bool>: Stop when minimum is attained (save computation time)
...
def test_stat(X,p=np.inf):
    ...
    In:  X (n,)    : X[j] = -log(1-F(|Y[j]|)) where F: cdf of |Y[j]| under null hypothesis
...
def null_distribution(n,nsimu):
    ...
    Syntax: S = null_distribution(n,nsimu)
