"""This file contains functions to display a large picture of a bunch of brain slices,
using pylab and pynifti.
It can be used as a module, or a standalone program.
In the later case, see the usage information at the __main__ below.
TODO : add an option for non-contour mode and for grid+markers mode

pynifti dependence removed now, but the resultstll has to be checked
"""
...
def resampleNiftiImage(src, target, order = 2, mode = 'constant'):
    ...
    #a = affine_transform(src.data.T, af, of, output_shape=target.data.T.shape,
    #                     order = order, mode = mode).T
    #return nifti.NiftiImage(a, target.header)
    a = affine_transform(src.data, af, of, output_shape=target.data.shape,
                         order = order, mode = mode).T
    nim = Nifti1Image(a, affine)
    return nim
...
of numerical values"""
    ijk = swapaxes(niImage.get_data(), 0, 2).astype(float)
    #axesnames = niImage.getSOrientation(True)
    axesnames = ['Left-to-Right', 'Posterior-to-Anterior', 'Inferior-to-Superior']
    templateijk = swapaxes((contourNiTemplate.get_data() if contourNiTemplate else niImage.get_data()), 0, 2) # avoid specialcasing
...
def display_norm_contour_manyfilenames(niTemplate, filenameStruct, filenameBold, views = [0, 1, 2], angles = [66, 48, 46]):
    ...
    niTemplate : an image defining the "template" size on which other images will be
    ...
    for j in range(N):
        ...
        try:
            niRStruct = resampleNiftiImage(load(filenameStruct[j]), niTemplate)
        except:
            print "Error opening/resampling %s"  % filenameStruct[j]
            #niRStruct = nifti.NiftiImage(zeros_like(niTemplate.data),niTemplate.header)
            niRStruct = Nifti1Image(zeros_like(niTemplate.get_data()),niTemplate.get_affine())
        ...
        try:
            ...
            niRBold = resampleNiftiImage(load(filenameBold[j]), niTemplate)
        except:
            print "Error opening/resampling %s"  % filenameBold[j]
            #niRBold = nifti.NiftiImage(zeros_like(niTemplate.data), niTemplate.header)
            niRBold = Nifti1Image(zeros_like(niTemplate.get_data()), niTemplate.get_affine())
        for i in range(len(views)):
