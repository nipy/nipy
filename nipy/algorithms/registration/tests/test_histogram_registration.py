...
def test_clamping_uint8():
    I = neuro_image(make_data_uint8(), dummy_affine, 'scanner')
    _test_clamping(I)
...
def test_clamping_int16():
    I = neuro_image(make_data_int16(), dummy_affine, 'scanner')
    _test_clamping(I)


def test_masked_clamping_int16():
    I = neuro_image(make_data_int16(), dummy_affine, 'scanner')
    mask = neuro_image(make_data_bool(), dummy_affine, 'scanner')
...
def test_clamping_float64():
    I = neuro_image(make_data_float64(), dummy_affine, 'scanner')
    _test_clamping(I)
...
def _test_similarity_measure(simi, val):
    I = neuro_image(make_data_int16(), dummy_affine, 'scanner')
    J = neuro_image(I.get_data().copy(), dummy_affine, 'scanner')
...
def test_explore():
    I = neuro_image(make_data_int16(), dummy_affine, 'scanner')
    J = neuro_image(make_data_int16(), dummy_affine, 'scanner')
    R = HistogramRegistration(I, J)
    T = Affine()
...
def test_histogram_masked_registration():
    """ Test the histogram registration class.
    """
    I = neuro_image(make_data_int16(dx=100, dy=100, dz=50), dummy_affine, 'scanner')
    J = neuro_image(make_data_int16(dx=100, dy=100, dz=50), dummy_affine, 'scanner')
    mask = (np.zeros((100,100,50)) == 1)
    mask[10:20,10:20,10:20] = True
    mask_img = neuro_image(mask, dummy_affine, 'scanner')
    R = HistogramRegistration(I, J, to_mask=mask_img, from_mask=mask_img)
    sim1 = R.eval(Affine())
    I = neuro_image(I.get_data()[mask].reshape(10,10,10), dummy_affine, 'scanner')
    J = neuro_image(J.get_data()[mask].reshape(10,10,10), dummy_affine, 'scanner')
    R = HistogramRegistration(I, J)
    sim2 = R.eval(Affine())
