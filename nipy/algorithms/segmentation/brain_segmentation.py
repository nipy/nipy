...
T1_ref_class_parameters['5k'] = {
    'mu': np.array([722.8, 1152.6, 1556.4, 1728.8, 2165.0]),
    'sigma': np.array([22570.7, 22844.6, 18808.2, 20793.6, 15554.7])}
T1_ref_class_parameters['glob_mu'] = 1643.2
T1_ref_class_parameters['glob_sigma'] = 252772.3
...
class BrainT1Segmentation(object):
    ...
    def __init__(self, data, mask=None, model='3k'):
        ...
        if mixmat.ndim == 2: 
            ...
            self.mixmat = mixmat
        elif model in T1_ref_class_parameters.keys():
            if model == '3k':
        ...
        if nclasses <= 5:
            key = str(mixmat.shape[0]) + 'k'
            self.ref_mu = T1_ref_class_parameters[key]['mu']
            self.ref_sigma = T1_ref_class_parameters[key]['sigma']
        else:
            self.ref_mu = np.linspace(T1_ref_class_parameters['3k']['mu'][0], 
                                      T1_ref_class_parameters['3k']['mu'][-1], 
                                      num=nclasses)
            self.ref_sigma = np.linspace(T1_ref_class_parameters['3k']['sigma'][0], 
                                         T1_ref_class_parameters['3k']['sigma'][-1], 
                                         num=nclasses)
        self.glob_mu = T1_ref_class_parameters['glob_mu']
        self.glob_sigma = T1_ref_class_parameters['glob_sigma']
