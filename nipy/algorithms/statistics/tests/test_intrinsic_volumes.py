...
def test_lips_wrapping():
    ...
    c = np.indices(b1.shape).astype(float)
    ...
    for box_shape, exp_ivs in [[(10,),(1,9)],
        ...
        c = np.indices(box_shape).astype(float)
...
def test_lips2_disjoint():
    ...
    c = np.indices((40,40)).astype(float)
    # Test N dimensional coordinates (N=10)
...
def test_lips3_disjoint():
    ...
    c = np.indices((40,)*3).astype(float)
    # Test N dimensional coordinates (N=10)
...
def test_lips3_nans():
    ...
    box2[7:22,0,8:17] = 1
    c = np.indices(box1.shape).astype(float)
...
def test_slices():
    ...
    D = np.indices(m.shape).astype(float)
    m[10,10,10] = 1
