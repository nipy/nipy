...
class Term(sympy.Symbol):
    ...
    >>> t = Term('x')
    >>> xval = np.array([(3,),(4,),(5,)], np.dtype([('x', float)]))
    >>> f = t.formula
    >>> d = f.design(xval)
    >>> print(d.dtype.descr)
    [('x', '<f8')]
...
def make_recarray(rows, names, dtypes=None, drop_name_dim=_NoValue):
    ...
    we cast the data in all columns in `rows` as float. If `dtypes` is not
    ...
    ...               [float, np.int])  #doctest: +STRUCTARR_EQUAL
    ...
    if dtypes is None:
        dtype = np.dtype([(n, float) for n in names])
    else:
...
class Formula(object):
    ...
    def _getdtype(self):
        ...
        return np.dtype([(n, float) for n in vnames])
    ...
    def _setup_design(self):
        ...
        self._dtypes = {'param':np.dtype([(str(p), float) for p in params]),
                        'term':np.dtype([(str(t), float) for t in terms]),
                        'preterm':np.dtype([(n, float) for n in preterm])}
    ...
    def design(self,
        ...
        return_float : bool, optional
           If True, return a float array rather than a np.recarray
        ...
        if list(self.terms) == [sympy.Number(1)]:
            a = np.ones(preterm_recarray.shape[0], float)
            if not return_float:
                a = a.view(np.dtype([('intercept', float)]))
            return a
        ...
        float_array = term_recarray.view(float)
...
def natural_spline(t, knots=None, order=3, intercept=False):
    ...
    >>> xval = np.array([3,5,7.]).view(np.dtype([('x', float)]))
...
class Factor(Formula):
    ...
    def fromcol(col, name):
        ...
        Examples
        --------
        >>> data = np.array([(3,'a'),(4,'a'),(5,'b'),(3,'b')], np.dtype([('x', float), ('y', 'S1')]))
