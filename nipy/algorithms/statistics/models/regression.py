...
class OLSModel(LikelihoodModel):
    ...
    >>> print(results.Tcontrast([0,1]))  #doctest: +FLOAT_CMP_6DP
    <T contrast: effect=2.14285714286, sd=1.14062281591, t=1.87867287326,
    df_den=5>
    >>> print(results.Fcontrast(np.eye(2)))  #doctest: +FLOAT_CMP_6DP
...
class ARModel(OLSModel):
    ...
    ...                              order=2,
    ...                              df=model.df_resid)
    ...     model = ARModel(model.design, rho) #doctest: +FLOAT_CMP_6DP
    ...
    AR coefficients: [ 0.  0.]
    ...
    >>> print(results.Fcontrast(np.identity(2)))  #doctest: +FLOAT_CMP_6DP
    ...
    >>> print(model.rho)  #doctest: +FLOAT_CMP_6DP
