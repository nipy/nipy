...
class FmriImageList(ImageList):
    ...
    def __init__(self, images=None, volume_start_times=None, slice_times=None):
        ...
        if v.shape == (len(self.list),):
            ...
            self.volume_start_times = np.arange(len(self.list)) * v
    ...
    def from_image(klass, fourdimage, volume_start_times=None, slice_times=None):
        ...
        """
        data = fourdimage.get_data()
        np.rollaxis(data, 3)
        images = [fourdimage.get_lookalike(d) for d in data]
        return klass(images=images, 
                     volume_start_times=range(data.shape[3]),
