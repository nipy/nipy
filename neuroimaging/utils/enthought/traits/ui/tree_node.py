...
# Date: 12/03/2004
# Description: Define the tree node descriptor used by the tree editor and tree
...
from neuroimaging.utils.enthought.traits.ui \
    import View

from neuroimaging.utils.enthought.resource \
...
class TreeNode ( HasPrivateTraits ):
    ...
    # Name to use when a new instance is being created:
    name = Str  
    ...
    # Automatically open ( i.e. expand) tree nodes?
    auto_open = false
    
    # Automatically close sibling tree nodes?
    auto_close = false
    
    # List of object classes than can be added/copied:
    add = List( Any )
    ...
    # Function for formatting label:
    formatter = Callable
    
    # Function for handling selecting an object: 
    on_select = Callable
    
    # Function for handling double_clicking an object: 
    on_dclick = Callable
    
    # View to use for editing object:
    view = Instance( View )
    ...
    # Resource path used to locate node icon:
    icon_path = Str   
    
    # fixme: The 'menu' trait should really be defined as:
    #        Instance( 'neuroimaging.utils.enthought.traits.ui.menu.MenuBar' ), but it doesn't work
    #        right currently.
...
#---- Overridable methods: -----------------------------------------------------
    ...
    def is_addable ( self, klass ):
        """ Returns whether or not a specified object class can be added to the
            node.
        """
...
class MultiTreeNode ( TreeNode ):
    ...
    # TreeNode that applies to base object itself:
