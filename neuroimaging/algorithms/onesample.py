...
class ImageOneSample(onesample.OneSampleIterator):
    ...
    def __init__(self, input, outputs=None, path='onesample', ext='.hdr',
        ...
        coordmap = self.iterator.imgs[0].coordmap
        if self.which == 'mean':
            if t:
                self.outputs.append(TOutput(coordmap, path=path,
                                            clobber=clobber, ext=ext))
            if sd:
                self.outputs.append(SdOutput(coordmap, path=path,
                                             clobber=clobber, ext=ext))
            if mean:
                self.outputs.append(MeanOutput(coordmap, path=path,
                                               clobber=clobber, ext=ext))
        else:
            if est_varatio:
                self.outputs.append(VaratioOutput(coordmap, path=path,
                                                  clobber=clobber, ext=ext))

            if est_varfix:
                self.outputs.append(VarfixOutput(coordmap, path=path,
                                                 clobber=clobber, ext=ext))
...
class ImageOneSampleOutput(RegressionOutput):
    ...
    def __init__(self, coordmap, nout=1, basename="", clobber=False,
        ...
        :Parameters:
            ...
            ext : ``string``
                TODO
        """
        RegressionOutput.__init__(self, coordmap, nout)
