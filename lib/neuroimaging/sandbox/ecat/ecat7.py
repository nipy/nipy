...
class ECAT7(bin.BinaryFormat):
    ...
    # Anything which should be default different
    # than field-defaults
    ...
    def __init__(self, filename, mode="r", \
                 datasource=DataSource(), **keywords):
        """
        ...
        bin.BinaryFormat.__init__(self, filename, mode, \
                                  datasource, **keywords)
        ...
        if self.mode[0] is 'w':
            ...
            self.read_header()
            self.generate_mlist()
        ### Read in Subheaders and Data
        number_frames = self.mlist.shape[1]
        self.sub_header = number_frames * [struct_formats_sh]
        self.subheader_defaults()
    ...
    def checkversion(self,datasource = DataSource()):
        ...
        magic_number = bin.struct_unpack\
                       (hdrfile,byteorder, field_formats_mh[0])[0]
        magic_number = magic_number[0]
        magic_number = magic_number.split('/x00')
        if magic_number[0]is not 'MATRIX72v':
            raise NotImplementedError\
                  ("%s ECAT version not supported"%magic_number[0])
    ...
    def guess_byteorder(hdrfile, datasource=DataSource()):
        ...
        if type(hdrfile)==type(""):
            ...
            reported_length = bin.struct_unpack(hdrfile,byteorder, \
                                                field_formats_mh[2])[0]
    ...
    def header_defaults(self):
        ...
        """
        for field,format in struct_formats_mh.items():
    ...
    def subheader_defaults(self):
        ...
        for i in range(len(self.sub_header)):
            for field,format in struct_formats_sh.items():
                self.sub_header[i][field]= self._default_sub_field_value(field,format)
