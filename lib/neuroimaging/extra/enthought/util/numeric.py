...
import scipy
from scipy import take, amin, amax, arange, asarray, PyObject, mean, std, isnan,\
                  product, shape, array, Float64, nonzero, isfinite, compress
...
def safe_flat(a):
    ...
    from neuroimaging.extra.enthought.util.numerix import which
    try:
        if which[0]=='numpy':
            safeflat = a.flatten()
        else:
