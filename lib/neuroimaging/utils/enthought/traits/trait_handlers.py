...
# Date: 06/21/2002
# Description: Define the base 'TraitHandler' class and a standard set of
#              TraitHandler subclasses for use with the 'traits' package.
#
#              A trait handler mediates the assignment of values to object
#              traits. It verifies (through the TraitHandler's 'validate'
#              method) that a specified value is consistent with the object
#              trait, and generates a 'TraitError' exception if not.
#
...
from types \
    import StringType, InstanceType, TypeType, FunctionType, MethodType
...
class TraitHandler ( object ):
    ...
    def get_editor ( self, trait ):
        ...
        if self.editor is None:
            from neuroimaging.utils.enthought.traits.ui import TextEditor
...
class TraitRange ( TraitHandler ):
    ...
    **weight** trait attribute, which must be a non-negative float value.
    ...
    def get_editor ( self, trait ):
        ...
        if auto_set is None:
            auto_set = True
        from neuroimaging.utils.enthought.traits.ui import RangeEditor
...
class TraitString ( TraitHandler ):
    ...
    If the value assigned to the trait attribute is a Python **int**, **long**,
    **float**, or **boolean**, the TraitString handler first coerces the value
    to a string. Values of other non-string types result in a TraitError being
    raised. The handler then makes sure that the resulting string is within
    the specified length range and that it matches the regular expression.
    """
...
class TraitType ( TraitHandler ):
    ...
    def get_editor ( self, trait ):
        ...
        if self.aType is bool:
            if self.editor is None:
                from neuroimaging.utils.enthought.traits.ui import BooleanEditor
...
class ThisClass ( TraitHandler ):
    ...
    def get_editor ( self, trait ):
        if self.editor is None:
            from neuroimaging.utils.enthought.traits.ui import InstanceEditor
...
class TraitInstance ( ThisClass ):
    ...
    TraitInstance is the underlying handler for the predefined trait `Instance`
    and the elements of `ListInstance`.
...
class TraitFunction ( TraitHandler ):
    ...
    TraitFunction is the underlying handler for the predefined trait `Function`,
    and for the use of function references as arguments to the `Trait()`
    function.
    """
...
class TraitEnum ( TraitHandler ):
    ...
    def get_editor ( self, trait ):
        from neuroimaging.utils.enthought.traits.ui import EnumEditor
...
class TraitCompound ( TraitHandler ):
    ...
    def get_editor ( self, trait ):
        from neuroimaging.utils.enthought.traits.ui import TextEditor, CompoundEditor
...
class TraitTuple ( TraitHandler ):
    ...
    TraitTuple is the underlying handler for the predefined trait `Tuple`, and
    the trait factory `Tuple()`.
    ...
    def get_editor ( self, trait ):
        from neuroimaging.utils.enthought.traits.ui import TupleEditor
...
class TraitList ( TraitHandler ):
    ...
    def get_editor ( self, trait ):
        ...
        if isinstance( handler, TraitInstance ) and (trait.mode != 'list'):
            from neuroimaging.utils.enthought.traits import HasTraits
            if issubclass( handler.aClass, HasTraits ):
                try:
                    object = handler.aClass()
                    from neuroimaging.utils.enthought.traits.ui.table_column import ObjectColumn
                    from neuroimaging.utils.enthought.traits.ui.table_filter import \
                         EvalFilterTemplate, RuleFilterTemplate, \
                         MenuFilterTemplate, EvalTableFilter
                    from neuroimaging.utils.enthought.traits.ui import TableEditor
                    return TableEditor(
        ...
        from neuroimaging.utils.enthought.traits.ui import ListEditor
...
class TraitDictEvent ( object ):
    ...
    def __init__ ( self, added = None, changed = None, removed = None ):
        """ added: a dict of new keys and values
            changed: a dict of updated keys and their previous values
            removed: a dict of old keys and values that were just removed
        """
...
class TraitDictObject ( dict ):
    ...
    def __setitem__ ( self, key, value ):
        ...
        try:
            ...
            return
        try:
            handler = self.trait.value_handler
            if handler is not None:
                value = handler.validate( object, self.name, value )
            if self.name_items is not None:
