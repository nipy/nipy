...
# Date: 10/07/2004
# Description: Define the UI class used to represent an active traits-based user
#              interface.
#
#  Symbols defined: UI
#
...
from neuroimaging.utils.enthought.traits.trait_base \
    import traits_home
...
# List of view 'kind' types which must have a 'parent' window specified:
...
class UI ( HasPrivateTraits ):
    ...
    # The ViewElements object this UI resolves Include items from:
    view_elements = view_elements_trait

    # Context objects the UI is editing:
    context = DictStrAny

    # Handler object used for event handling:
    handler = ReadOnly

    # View template used to construct the UI:
    view = ReadOnly

    # Panel/Dialog associated with the UI:
    control = Any

    # Toolkit specific object which 'owns' 'control':
    owner = Any

    # UIInfo object containing context/editor objects:
    info = ReadOnly

    # Result from a modal/wizard dialog:
    result = false

    # Undo/Redo history:
    history = Any

    # The unique 'persistence' id for this UI:
    id = Str

    # Have any modifications been made to UI contents?
    modified = false

    # User interface has changed event:
    updated = Event( Bool )

    # Title of dialog, if any:
    title = Str
    ...
    # Original context when used with modal dialog:
    _context = DictStrAny

    # Copy of original context used for 'revert':
    _revert = DictStrAny

    # List of methods to call once user interface created:
    _defined = List
    ...
    # List of names bound to 'info' object:
    _names = List

    # Index of currently active group in the ui:
    _active_group = Int

    # List of top-level groups used to build the ui:
    _groups = Property

    # Undoable action nesting level count:
    _undoable = Int( -1 )

    # Code used to rebuild an updated ui:
    _rebuild = Callable
    ...
    def ui ( self, parent, kind ):
        ...
        if (parent is None) and (kind in kind_must_have_parent):
            kind = 'live'
        self._rebuild = getattr( toolkit(), 'ui_' + kind )
        self._rebuild( self, parent )
    ...
    def _updated_changed ( self ):
        if self._rebuild is not None:
            ...
            if self.control is not None:
                ...
                parent = self.control._parent
                self.dispose( abort = True )
                self.info.ui = self
            self._rebuild( self, parent )
...
class Dispatcher ( object ):
    ...
    def __init__ ( self, method, info, object, method_name ):
        ...
        object.on_trait_change( self.dispatch, method_name )
