...
#  Date: 07/01/2005
#
#  Description: Define the table column descriptor used by the numeric and 
#               table editor and editor factory classes.
#
...
from neuroimaging.utils.enthought.traits.ui.menu \
    import Menu
    
from neuroimaging.utils.enthought.pyface.grid.grid_cell_renderer \
...
class ObjectColumn ( TableColumn ):
    ...
    def get_value ( self, object ):
        ...
        """
        try:
            return self.format % getattr( self.get_object( object ), self.name )
        except:
            return 'Format!'
    ...
    def cmp ( self, object1, object2 ):
        """ Returns the result of comparing the column of two different objects.
        """
        return cmp( getattr( self.get_object( object1 ), self.name ), 
                    getattr( self.get_object( object2 ), self.name ) )
...
class NumericColumn ( ObjectColumn ):
    ...
    # Cell value formatting string:
    format = Str( '%s' )
    
    # Horizontal alignment of text in the column (default override):
...
class ListColumn ( TableColumn ):
    ...
    # Is this column editable (overrides base class)?
    editable = False
