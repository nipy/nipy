...
def code_editor ( ):
    ...
    if SourceCodeEditor is None:
        from neuroimaging.utils.enthought.traits.ui import CodeEditor
...
def html_editor ( ):
    ...
    if HTMLTextEditor is None:
        from neuroimaging.utils.enthought.traits.ui import HTMLEditor
...
def shell_editor ( ):
    ...
    if PythonShellEditor is None:
        from neuroimaging.utils.enthought.traits.ui import ShellEditor
...
class CTrait ( cTrait ):
    ...
    def get_editor ( self ):
        from neuroimaging.utils.enthought.traits.ui import EditorFactory
...
def _Array ( typecode = None, shape = None, value = None, coerce = False, 
    ...
    if shape is not None:
        ...
        if (len( shape ) == 2) and (metadata.get( 'editor' ) is None):
            from neuroimaging.utils.enthought.traits.ui import ArrayEditor
...
def WeakRef ( klass = 'neuroimaging.utils.enthought.traits.HasTraits', allow_none = False, 
              **metadata ):
    """ Values must be instances of the same type (or a subclass) of the
...
def File ( value = '', filter = None, auto_set = False, **metadata ):
    from neuroimaging.utils.enthought.traits.ui.editors import FileEditor
...
def Directory ( value = '', auto_set = False, **metadata ):
    from neuroimaging.utils.enthought.traits.ui.editors import DirectoryEditor
...
def Button ( label = '', image = None, style = 'button', 
             ...
             **metadata ):
    from neuroimaging.utils.enthought.traits.ui import ButtonEditor
...
def UIDebugger ( **metadata ):
    from neuroimaging.utils.enthought.traits.ui.wx.ui_debug_editor import ToolkitEditorFactory
...
def Color ( *args, **metadata ):
    from neuroimaging.utils.enthought.traits.ui import ColorTrait
    return ColorTrait( *args, **metadata )
    
Color = TraitFactory( Color )

def RGBColor ( *args, **metadata ):
    from neuroimaging.utils.enthought.traits.ui import RGBColorTrait
    return RGBColorTrait( *args, **metadata )
    
RGBColor = TraitFactory( RGBColor )

def RGBAColor ( *args, **metadata ):
    from neuroimaging.utils.enthought.traits.ui import RGBAColorTrait
    return RGBAColorTrait( *args, **metadata )
    
RGBAColor = TraitFactory( RGBAColor )
    
def Font ( *args, **metadata ):
    from neuroimaging.utils.enthought.traits.ui import FontTrait
    return FontTrait( *args, **metadata )
    
Font = TraitFactory( Font )

def KivaFont ( *args, **metadata ):
    from neuroimaging.utils.enthought.traits.ui import KivaFontTrait
