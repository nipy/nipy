...
has_traits_dealloc ( has_traits_object * obj ) {
    ...
    Py_DECREF(  obj->ctrait_dict );
    Py_XDECREF( obj->itrait_dict );
    Py_XDECREF( obj->notifiers );
    Py_XDECREF( obj->obj_dict );
...
trait_dealloc ( trait_object * trait ) {
    ...
    Py_XDECREF( trait->py_post_setattr );
    Py_XDECREF( trait->delegate_name );
    Py_XDECREF( trait->delegate_prefix );
    Py_XDECREF( (PyObject *) trait->notifiers );
    Py_XDECREF( trait->handler );
    Py_XDECREF( trait->obj_dict );
...
_trait_cast ( trait_object * trait, PyObject * args ) {
    ...
    switch ( PyTuple_GET_SIZE( args ) ) {
        ...
        default:
            PyErr_Format( PyExc_TypeError, 
                "Trait cast takes 1, 2 or 3 arguments (%d given).", 
...
trait_method_traverse ( trait_method_object * tm, visitproc visit, 
	...
	if ( tm->tm_func ) {
		err = visit( tm->tm_func, arg );
		if ( err )
			return err;
	}
	if ( tm->tm_self ) {
		err = visit( tm->tm_self, arg );
		if ( err )
			return err;
	}
	if ( tm->tm_traits ) {
		err = visit( tm->tm_traits, arg );
		if ( err )
			return err;
	}
	if ( tm->tm_class ) {
		err = visit( tm->tm_class, arg );
		if ( err )
			return err;
	}
	return 0;
...
initctraits ( void ) {
	...
	has_traits_type.tp_free  = _PyObject_Del;
	...
	trait_type.tp_free  = _PyObject_Del;
