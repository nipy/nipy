...
class RFXMean(traits.HasTraits):
    ...
    def __init__(self, input, df=None, fwhm=None, tol=1.0e+05, outputs=None, **keywords):
        ...
        self.X = N.ones((self.nsubject, 1), N.Float)
        ...
        if self.df_resid > 0:
            if sd_files:
                try:
                    ...
                    self.input_df = N.ones((len(input_files),), N.Float) * df
            else:
	...
	effect = N.zeros(Y.shape[1:], N.Float)
        ...
        Sigma = self.S + multiply.outer(N.ones((self.nsubject,), N.Float), sigma2)
        ...
        Sigma = N.dot(N.one((self.nsubject,), N.Float), sigma2)
        W = recipr(Sigma)
